...
describe("Electron runtime efficiency policy", () => {
  ...
  it("finds no polling interval or undeclared production timer owner", async () => {
    ...
    expect(report.ownerFiles).toContain("src/kafka/ui/TrustRecipeManager.tsx");
    expect(report.ownerFiles).toContain("src/main/electron-event-delivery.ts");
    expect(report.ownerFiles).toContain("src/kafka/application/trust-acquisition-service.ts");
  });
});
